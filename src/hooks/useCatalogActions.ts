...
export function useCatalogActions(
  ...
  const filteredBuilds = useMemo(() => {
    ...
    if (filters.installedOnly) {
      result = result.filter((b) =>
        installedKeys.has(makeKey(b.build_number, b.backend, b.architecture))
      );
    }
    ...
    async (build: Build) => {
      ...
      if (hasActiveDownload) {
        toast({
          ...
          description: 'Only one download at a time is allowed.',
        });
        return;
      }
      ...
      const alreadyInstalled = installed?.find(
        (v) =>
          v.build_number === build.build_number &&
          v.backend === build.backend &&
          v.architecture === build.architecture &&
          v.status === 'installed'
      );
      if (alreadyInstalled) {
        toast({
          title: 'Already installed',
          description: `${build.build_number} (${build.backend} ${build.architecture}) is already installed.`,
        });
        return;
      }
      ...
      try {
        ...
        toast({
          ...
          description: err instanceof Error ? err.message : 'Could not start download.',
        });
        const store = useAppStore.getState();
  ...
  const toggleBackendFilter = useCallback(
    (backend: string) => {
