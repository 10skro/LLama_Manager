...
export function SettingsPage() {
  ...
  // Intentionally unused, reserved for future refresh feature
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label className="flex items-center gap-2">
                ...
                <Badge
                  variant="outline"
                  className="text-yellow border-yellow/30 bg-yellow/10 text-xs"
                >
                  WIP
                </Badge>
              </Label>
                  ...
                  onChange={(e) => {
                    ...
                    if (settings) {
                      modelFolderSaveTimerRef.current = setTimeout(() => {
                        saveSettings({ ...settings, model_folder: val || undefined }).catch(
                          (err) => {
                  ...
                  onChange={(e) => {
                    ...
                    if (settings) {
                      mmprojFolderSaveTimerRef.current = setTimeout(() => {
                        saveSettings({ ...settings, mmproj_folder: val || undefined }).catch(
                          (err) => {
                        ...
                        if (settings) {
                          saveSettings({ ...settings, theme: theme.id }).catch((err) => {
                            ...
                            toast({
                              title: 'Save failed',
                              description:
                                'Could not persist theme. Changes will be lost on restart.',
                              variant: 'destructive',
                            });
                          });
                        }
                  ...
                  return (
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (settings) {
                          saveSettings({ ...settings, font_family: font.cssFamily }).catch(
                            (err) => {
                              console.error('Failed to auto-save font:', err);
                              toast({
                                title: 'Save failed',
                                description:
                                  'Could not persist font. Changes will be lost on restart.',
                                variant: 'destructive',
                              });
                            }
                          );
                  ...
                  if (settings) {
                    ...
                    toast({
                      ...
                      description: val
                        ...
                        : "Won't check for updates on startup.",
                    });
                  }
                }}
              >
                ...
                onClick={() => {
                  if (settings) {
                    ...
                    toast({
                      ...
                      description: val
                        ? 'Will show changelog on startup when update available.'
                        : "Won't show changelog modal on startup.",
                    });
                  }
                }}
              >
                        ...
                        if (settings) {
                          saveSettings({ ...settings, toast_duration: opt.value }).catch((err) => {
                            ...
                            toast({
                              title: 'Save failed',
                              description:
                                'Could not persist toast duration. Changes will be lost on restart.',
                              variant: 'destructive',
                            });
                          });
                        }
              ...
              >
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Optional: Add a personal access token to increase the GitHub API rate limit
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <div className="relative flex-1">
                        <Input
                          ...
                          placeholder={
                            hasToken ? 'Token is configured (edit to update)' : 'ghp_...'
                          }
                      ...
                      {githubToken.length > 0 && (
                        <Button size="sm" onClick={handleSaveToken} disabled={isSavingToken}>
                          {isSavingToken ? (
                            <Loader2 className="h-4 w-4 animate-spin" />
                          ) : (
                            <Save className="h-4 w-4" />
                    ...
                    <p className="text-xs text-muted-foreground">
                      Your token is stored locally and only used for GitHub API requests. Create a
                      token at{' '}
                      <a
